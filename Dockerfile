...
RUN apt-get update && apt-get -y install xserver-xorg-video-dummy x11vnc xdotool wget tar supervisor net-tools fluxbox gnupg2 python-numpy x11-xserver-utils
...
WORKDIR /root/
RUN wget -O - https://github.com/novnc/noVNC/archive/v1.1.0.tar.gz | tar -xzv -C /root/ && mv /root/noVNC-1.1.0 /root/novnc && ln -s /root/novnc/vnc.html /root/novnc/index.html
